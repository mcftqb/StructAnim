...
$data modify storage struct_anim:utils root.args.str.concat.values append value '{"text":"Animation Area Corners","bold":true,"color":"gold","clickEvent": {"action": "run_command","value": "$(on_click)"},"hoverEvent":{"action":"show_item","value":"{id:\\"structure_block\\", Count:1b, tag:'
data modify storage struct_anim:utils root.args.str.escape.value set from storage struct_anim:utils root.return.itemset.animation.new.corner.value
